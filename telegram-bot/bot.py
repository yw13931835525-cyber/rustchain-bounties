...
from telegram.ext import (
    ...
    filters, ContextTypes, RateLimiter
)
...
def api_get(endpoint: str) -> Optional[dict]:
    """Fetch JSON from the RustChain node."""
...
async def cmd_miners(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data is None:
        await update.message.reply_text("❌ Node unreachable. Try again later.")
        return

    miners = data.get("miners", [])
    total  = data.get("pagination", {}).get("total", len(miners))

    # Sort by antiquity multiplier
